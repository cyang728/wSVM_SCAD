scadsvc <- function(lambda1 = 0.01, x, y, a = 3.7, tol = 10^(-4), sample.weights = w_hat,
  ...
  while (diff > tol) {
    ...
    D_vec <- sample.weights / (2 * ntrain) * (1 / res)
    aw <- abs(w)
    dp <- lambda1 * (aw <= lambda1) + (a * lambda1 - aw) / (a - 1) * (lambda1 < aw & aw <= a * lambda1)
    Q1_vec <- c(0, dp / aw)
    
    P <- 0.5 * t(ytrain + y0) %*% diag(sample.weights) %*% x1 / ntrain
  ...
  if (sum(!is.na(ind)) > 0 & sum(ind) > 0) {
    ...
    #BIC <- sum(sample.weights * pmax(0, 1 - ytrain * f)) + length(xind) * log(sum(sample.weights))
    BIC <- sum(pmax(0, 1 - ytrain*f)) + length(xind)*log(length(ytrain))
